...
class Metric3DV2Wrapper(nn.Module):
    ...
    def __init__(self, model_variant="vit_small"):
        ...
        if model_variant not in ("vit_small", "vit_base", "vit_large"):
            raise ValueError(f"Unknown model variant: {model_variant}. Available variants: {list(variant_map.keys())}")
    ...
    def forward(self, img):
        ...
        Args:
            img (torch.Tensor): Input image tensor in [C,H,W] format, range [0,255]
        ...
        rgb_origin = img.permute(1, 2, 0).cpu().numpy()  # [H,W,C]
