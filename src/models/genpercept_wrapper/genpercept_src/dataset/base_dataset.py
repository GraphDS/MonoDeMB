...
class BaseDataset(Dataset):
    ...
    def _get_data_item(self, index):
        ...
        print('rgb_rel_path', rgb_rel_path,
              'depth_rel_path', depth_rel_path,
              'filled_rel_path', filled_rel_path,
              'normal_rel_path', normal_rel_path,
              'matting_rel_path', matting_rel_path,
              'dis_rel_path', dis_rel_path,
              'seg_rel_path', seg_rel_path)

        rasters = {}

        # RGB data
        rasters.update(self._load_rgb_data(rgb_rel_path=rgb_rel_path)) #rgb_int rgb_norm"
        print("rgb_int", rasters["rgb_int"].min(), rasters["rgb_int"].max())
        print("rgb_norm", rasters["rgb_norm"].min(), rasters["rgb_norm"].max())

        if DatasetMode.RGB_ONLY != self.mode:
            # Depth
            print('5728047203489023840923840823904823048023840923')
            if depth_rel_path is not None:
                print("DEPTH   " * 5, depth_rel_path) 
            ...
            if self.has_filled_depth:
                print('if self.has_filled_depth   ' * 3)
                rasters["valid_mask_filled"] = self._get_valid_mask(
                    rasters["depth_filled_linear"]
                ).clone()
                print("valid_mask_filled", rasters["valid_mask_filled"].min(), rasters["valid_mask_filled"].max())
    ...
    def _load_depth_data(self, depth_rel_path, filled_rel_path, shape=None):
        ...
        outputs = {}
        try:
            print('TRY TRY _load_depth_data')
        ...
        if self.has_filled_depth:
            print('self.has_filled_depth  _load_depth_data')
...
#         print('self.filenames[index]', self.filenames[index])
    ...
    def _read_image(self, img_rel_path, convert_rgb=False) -> np.ndarray:
        ...
        print(f"self.is_tar {self.is_tar}, image_to_read.endswith'.exr' {image_to_read.endswith('.exr')} convert_rgb {convert_rgb}")
        return image
    ...
    def _get_valid_mask(self, depth: torch.Tensor):
        print('base_dataset,_get_valid_mask', self.min_depth, self.max_depth)
